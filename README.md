...
These numbers are verifier-graded, so on their own they rank the retrievers against each other and not against truth. The 26 disagreements now have labels, in [`samples/spot-check-labels.md`](samples/spot-check-labels.md): each read against the segment's actual transcript, which is evidence the verifier never gets. They were written by a model rather than by hand, and that file says plainly what that is and is not worth. Of the 26 that pass here, 11 are right — 9 of the 21 the embedding retriever found alone, 2 of the 5 keyword search found alone — so nothing in the table above should be read as precision. Re-scoring also moved the pipeline's own matches: 38 of the 45 passed again, and 5 of the 7 that did not were the score-7 false positives named below.
...
**What the labels say.** Every disagreement in this section is labelled in [`samples/spot-check-labels.md`](samples/spot-check-labels.md) by reading the segment's transcript instead of its one-sentence description. Seven of those eight embedding-only finds are right. The one that is not is The Daily on kill switches — the one the paragraph above already said it would argue with. Across the whole set the two changes compound:

| Verifier, cutoff | Disagreements passed | Correct | Precision |
|---|---|---|---|
| Production, 7 (as shipped) | 26 | 11 | 42% |
| Production, 8 | 20 | 10 | 50% |
| Context, 7 | 16 | 8 | 50% |
| Context, 8 (current default) | 8 | 7 | 88% |

That is 27 labelled pairs, and they are the hard subset by construction: the segments the two retrievers disagreed about. The candidates both retrievers found are not labelled, so none of these numbers is the pipeline's precision.

The stricter prompt also loses two matches I would keep: the Tech Brew segment on Hugging Face's robot as the reason for Nvidia's price, which is analysis of the deal rather than the deal, and one Steinem segment about a first meeting in 1993. Both looked like fair calls either way, and the labels split them. The Steinem segment is #8 in the labelled set and is marked right, and the context verifier drops it, so that one is a real recall loss rather than a fair call. The Tech Brew segment both retrievers found, so it is not a disagreement and is not in the labelled set.
...
- **No tests, no eval set.** Quality was judged by reading output, which is how every threshold above was chosen. The retrieval comparison is the first measurement in the repository, and the defaults were changed on its evidence, but it is graded by the same verifier, so it measures the two retrievers against each other rather than against truth; its disagreements are now labelled against the transcripts ([`samples/spot-check-labels.md`](samples/spot-check-labels.md)), which turns the 7-versus-8 question into a number: 42% correct as shipped, 88% at the current default. But those are 27 model-written labels over the subset the retrievers disagreed about, not hand labels and not the pipeline's precision. What this still needs is a hundred human-labelled (story, segment) pairs drawn from the agreements as well.
